...
model = Sequential()
model.add(layers.Bidirectional(layers.LSTM(512, input_shape=(431, 1025), return_sequences=True, dropout=0.2),
                               merge_mode='concat'))
model.add(layers.Dense(1025))

cp_callback = callbacks.ModelCheckpoint(filepath=r'checkpoints/ModelMag10/ModelMag10-best.ckpt',
                                        save_weights_only=True, monitor='val_loss', mode='min',
                                        save_best_only=True, verbose=1, save_freq='epoch')

csv_logger = callbacks.CSVLogger(r'checkpoints/ModelMag10/ModelMag10-log.csv')
