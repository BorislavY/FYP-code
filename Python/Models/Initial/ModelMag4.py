...
model = Sequential()
model.add(layers.Bidirectional(layers.LSTM(256, input_shape=(431, 1025), return_sequences=True), merge_mode='sum'))
...
cp_callback = callbacks.ModelCheckpoint(filepath=r'checkpoints\ModelMag4\ModelMag4-best.ckpt',
                                        save_weights_only=True, monitor='val_loss', mode='min',
                                        save_best_only=True, verbose=1, save_freq='epoch')

csv_logger = callbacks.CSVLogger(r'checkpoints\ModelMag4\ModelMag4-log.csv')
